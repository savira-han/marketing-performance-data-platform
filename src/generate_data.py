...
# ------------------------------------------------------------
# 9.5 Save updated customer dimension
# ------------------------------------------------------------
...
# ------------------------------------------------------------
#
# df_customer_final was created in Section 9 and now contains:
#
# customer_id
# country
# signup_date
...
dim_customer = df_customer_final.copy()
...
print(
    f"✓ dim_customer.csv created "
    f"({len(dim_customer):,} rows)"
)


# ------------------------------------------------------------
# 10.2 CHANNEL DIMENSION
# ------------------------------------------------------------
#
# Grain: 1 row per channel
# ------------------------------------------------------------

dim_channel = df_channels.copy()
...
print(
    f"✓ dim_channel.csv created "
    f"({len(dim_channel):,} rows)"
)


# ------------------------------------------------------------
# 10.3 CAMPAIGN DIMENSION
# ------------------------------------------------------------
#
# Grain: 1 row per campaign
#
# channel_id is retained as the foreign key to dim_channel.
# ------------------------------------------------------------

dim_campaign = df_campaigns.copy()
...
print(
    f"✓ dim_campaign.csv created "
    f"({len(dim_campaign):,} rows)"
)
...
# ------------------------------------------------------------
#
# Create one row for every calendar date in 2025.
#
# This gives the analytical model a consistent definition
# of calendar periods for future reporting.
# ------------------------------------------------------------
...
dim_date = pd.DataFrame(
    {
        ...
        "day_of_week_number": date_range.dayofweek + 1
    }
)
...
print(
    f"✓ dim_date.csv created "
    f"({len(dim_date):,} rows)"
)


# ------------------------------------------------------------
# 10.5 MARKETING TOUCHPOINT FACT
# ------------------------------------------------------------
#
# Grain: 1 row per marketing touchpoint
#
# Direct / Organic touchpoints remain in this fact table.
# Their campaign_id is NULL because they are not attributable
# marketing campaigns.
# ------------------------------------------------------------

fact_marketing_touchpoint = df_touchpoints.copy()
...
print(
    f"✓ fact_marketing_touchpoint.csv created "
    f"({len(fact_marketing_touchpoint):,} rows)"
)
...
#
# attributed_campaign_id is booking-level attribution.
# It can differ from the customer's acquisition campaign.
# ------------------------------------------------------------

fact_booking = df_bookings.copy()
...
print(
    f"✓ fact_booking.csv created "
    f"({len(fact_booking):,} rows)"
)


# ------------------------------------------------------------
# 10.7 AD PERFORMANCE FACT
# ------------------------------------------------------------
#
# Grain: 1 row per campaign per day
# ------------------------------------------------------------

fact_ad_performance = df_ad_performance.copy()
...
print(
    f"✓ fact_ad_performance.csv created "
    f"({len(fact_ad_performance):,} rows)"
)


# ------------------------------------------------------------
# 10.8 PROCESSED TABLE VALIDATION
# ------------------------------------------------------------
...
print("✓ dim_customer grain validated")


# Channel grain
assert (
    dim_channel["channel_id"].nunique()
    == len(dim_channel)
)

print("✓ dim_channel grain validated")


# Campaign grain
assert (
    dim_campaign["campaign_id"].nunique()
    == len(dim_campaign)
)

print("✓ dim_campaign grain validated")
...
assert len(dim_date) == 365

print("✓ dim_date grain validated")


# Touchpoint grain
assert (
    fact_marketing_touchpoint["touchpoint_id"].nunique()
    == len(fact_marketing_touchpoint)
)

print("✓ fact_marketing_touchpoint grain validated")


# Booking grain
assert (
    fact_booking["booking_id"].nunique()
    == len(fact_booking)
)

print("✓ fact_booking grain validated")


# Ad performance grain
assert (
    fact_ad_performance[
        [
            "campaign_id",
    ...
    .drop_duplicates()
    .shape[0]
    == len(fact_ad_performance)
)

print("✓ fact_ad_performance grain validated")


# ------------------------------------------------------------
# 10.9 FOREIGN KEY VALIDATION
# ------------------------------------------------------------
...
acquisition_campaign_ids = set(
    dim_customer[
        "acquisition_campaign_id"
    ].dropna()
)

assert acquisition_campaign_ids.issubset(
    valid_campaign_ids
)

print(
    "✓ Customer acquisition campaign FK validated"
)
...
assert booking_customer_ids.issubset(
    valid_customer_ids
)

print(
    "✓ Booking customer FK validated"
)


# Booking → attributed campaign
booking_campaign_ids = set(
    fact_booking[
        "attributed_campaign_id"
    ].dropna()
)

assert booking_campaign_ids.issubset(
    valid_campaign_ids
)

print(
    "✓ Booking attributed campaign FK validated"
)
...
campaign_channel_ids = set(
    dim_campaign["channel_id"]
)

assert campaign_channel_ids.issubset(
    valid_channel_ids
)

print(
    "✓ Campaign channel FK validated"
)


# Touchpoint → customer
touchpoint_customer_ids = set(
    fact_marketing_touchpoint[
        "customer_id"
    ]
)

assert touchpoint_customer_ids.issubset(
    valid_customer_ids
)

print(
    "✓ Touchpoint customer FK validated"
)
...
print(
    "✓ Touchpoint campaign FK validated"
)


# Ad performance → campaign
ad_campaign_ids = set(
    fact_ad_performance[
        "campaign_id"
    ]
...
assert ad_campaign_ids.issubset(
    valid_campaign_ids
)

print(
    "✓ Ad performance campaign FK validated"
)


# ------------------------------------------------------------
# 10.10 FINAL TABLE SUMMARY
# ------------------------------------------------------------
...
processed_tables = {
    ...
    "dim_campaign": len(dim_campaign),
    "dim_date": len(dim_date),
    "fact_marketing_touchpoint":
        len(fact_marketing_touchpoint),
    "fact_booking": len(fact_booking),
    "fact_ad_performance": len(fact_ad_performance)
}

for table_name, row_count in processed_tables.items():
    print(
        f"  {table_name:<30} "
        f"{row_count:>10,} rows"
    )
...
print(
    "✓ SECTION 10 PROCESSED TABLES COMPLETED"
)
